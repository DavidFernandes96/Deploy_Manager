...
public class Controller implements Runnable {
	...
	private static Queue<Replica> queue; // queue with replicas to soon join the pool
	...
	private Controller() {
		commands = new String[] { "vagrant up --no-provision", "vagrant destroy --force", "vagrant status",
				"vagrant halt ", "vagrant up ", "vagrant reload --provision " };
	...
	public int init() {
		try {
			...
			try {
				...
				minReplicasRunning = (Replica.getNumberReplicas() - ((Replica.getNumberReplicas() - 1) / 3)) != Replica
						.getNumberReplicas() ? (Replica.getNumberReplicas() - ((Replica.getNumberReplicas() - 1) / 3))
								: Replica.getNumberReplicas() - 1;
				FileWriter fileWrite = new FileWriter("Vagrantfile", true);
				BufferedWriter bw = new BufferedWriter(fileWrite);
				bw.write("end");
				bw.close();
		...
		while (!t1.isInterrupted()) {
			if (queue.size() > 0 && config.size() > minReplicasRunning) {
				...
				if (config.contains(replica)) {
					ProcessBuilder processBuilder = new ProcessBuilder();
					processBuilder.command("bash", "-c", commands[3] + replica.getName());
					try {
						Process process = processBuilder.start();
						process.waitFor();
						System.out.println("\n\nRemoved one replica (" + replica.getName() + ")");
						replica.setStatus(false);
						pool.add(replica);
						config.remove(replica);
